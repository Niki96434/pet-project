...
touch .env
```
5. В файле .env создаем секреты для access и refresh токенов, домен источника(клиента), порт,на котором запущен сервер и данные для подключения к БД PostgresSQL
```
PORT=3000
